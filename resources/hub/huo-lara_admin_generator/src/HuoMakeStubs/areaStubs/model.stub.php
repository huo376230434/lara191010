...
class DummySysArea extends BaseModel
{
    ...
    {
        if ($province_id) {
            return static::where('parent_id', $province_id)->where('node_id',1)->get()->toArray();

        }else{
            return static::where('node_id', 1)->get()->toArray();
        }
    }
    ...
    {
        if ($city_id) {
            return static::where('parent_id', $city_id)->where('node_id',2)->get()->toArray();

        }else{
            return static::where('node_id', 2)->get()->toArray();
    ...
    {
        return collect(static::provinces())->pluck('name', 'id')->toArray();

    }

    public static function cityOptions($province_id)
    {
        return static::where('parent_id',$province_id)->pluck('name', 'id')->toArray();

    }

    public static function districtOptions($city_id)
    {
        return static::where('parent_id',$city_id)->pluck('name', 'id')->toArray();
    ...
    {
        return $this->belongsTo(DummySysArea::class, 'parent_id');
    ...
    public static function allAreas()
    {
        ...
        return \Cache::remember('all_DummyAreaTable',10,function(){
...
//    {
//        return \Cache::remember('all_DummyAreaTable_with_country',100,function(){
//            return [
...
//    {
//            return DummySysArea::find($this->parent_id);
//    }
