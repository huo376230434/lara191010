<?php
namespace Huojunhao\Lib\Database;
...
class MysqlMaintenance{
    ...
    public function __construct($mysql_conf=[])
    {
        ...
        $this->save_path =isset($mysql_conf['database_backup_path']) ? Str::finish($mysql_conf['database_backup_path'],"/") : storage_path("mysql_backup/");
//        if(!$this->save_path){
...
//        将文件中的权限相关删除,方便直接用
        ...
        $content =  str_replace( $this->toReplaceStrWhenSql(), '', $content);

        file_put_contents($path, $content);
    }

    protected function toReplaceStrWhenSql()
    {
        return 'DEFINER=`d5pay`@`%`';
    }
